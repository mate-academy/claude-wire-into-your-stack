...
I attempted to use it on a real task with `claude -p`, but this environment is currently not authenticated (`Not logged in · Please run /login`). The command prepared for verification is:
...
## 5) Headless task run
I prepared one headless task with a tight allowed-tools list:
...
Execution is currently blocked in this environment until Claude CLI authentication is completed.
