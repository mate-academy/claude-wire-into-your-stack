...
Did not use gh auth login to access github, used on the MCP Server GITHUB. Performed the following opperations for this exersize leveraging
Claude code to perform all opperations:
git commit for all changes to project
...
- **`list-todos`** (`.claude/skills/list-todos/SKILL.md`) — Scans the tracked source for
  ...
  This was executed and return zero TODOs in the codebase. Modified the eslint.config.js
  Return the following report:
  '''
  TODO Report
  Total: 1 marker found in 1 file (a few other matches were prose about TODOs, not actual marker comments — see note below)

  eslint.config.js
  L3: TODO: Add a description of this config file.
  Excluded from the count: matches in NOTES.md, .claude/scan-prompt.txt, and .claude/skills/list-todos/SKILL.md were not skipped by mistake — they're documentation/prose that mentions the word "TODO" (e.g., describing this skill or a scan template), not actual // TODO:-style marker comments left in source code.
  '''
...
- **`/format`** (`.claude/commands/format.md`) — Reformats all project files with Prettier
  ...
  Ran format on project and it returned the following report:
  '''
  Prettier ran successfully after fixing the blocker. 
  Summary:
  Fixed to unblock formatting: eslint.config.js had a malformed block-comment terminator 
  (* / instead of */) that caused a syntax error and made Prettier fail on that file. I 
  fixed the typo so Prettier could parse and format it.

  Files Prettier reformatted (content changes):
    CLAUDE.md
    NOTES.md
    README.md
    SCAN_REPORT.md
  ...
  A handful of other files (.claude/commands/format.md, .github/workflows/ci.yml, package.json, 
  package-lock.json, .claude/skills/list-todos/SKILL.md) show as modified in git status but 
  have no actual content diff — that's just CRLF/LF line-ending normalization noise from git, 
  not a Prettier change.

  No lint or tests were run, per the command's scope.
  '''
## Hooks
